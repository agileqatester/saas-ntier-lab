...
**Pooled tenants (same platform):** namespaces `tenant-a` / `tenant-b` on one shared EKS cluster, one shared Postgres — no stack-per-tenant, no database-per-tenant. Isolation is enforced at four independent layers — network, identity, data, and compute — so a gap in one doesn't collapse the whole boundary; see [Pooled tenants](#pooled-tenants) below for the full breakdown. The public ALB is path-based on the default ELB DNS name — `/tenant-a*` → NodePort **30080**, `/tenant-b*` → **30081** — so no purchased domain is required. `helm/test-app/onboard-tenant.sh` handles the k8s half of onboarding; IAM/secrets stay in OpenTofu `for_each` so a new tenant is one `for_each` entry plus one script invocation, not a hand-wired stack.
